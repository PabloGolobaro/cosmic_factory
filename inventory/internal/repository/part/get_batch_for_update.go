...
func (s *store) GetBatchForUpdate(ctx context.Context, filter model.PartFilter) ([]entity.Part, error) {
	var (
		sqlStr string
		args   []any
	)
	...
	case len(filter.UUIDs) > 0:
		sqlStr = selectPartCols + ` WHERE uuid = ANY($1) ORDER BY uuid FOR UPDATE`
		args = []any{filter.UUIDs}
	case filter.PartType != valueobject.PartTypeUnspecified:
		sqlStr = selectPartCols + ` WHERE part_type = $1 ORDER BY uuid FOR UPDATE`
		args = []any{string(filter.PartType)}
	default:
		sqlStr = selectPartCols + ` ORDER BY uuid FOR UPDATE`
	}
